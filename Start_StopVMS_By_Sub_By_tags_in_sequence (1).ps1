...
.DESCRIPTION  
 Wrapper script for start & stop Az VM's
.EXAMPLE  
.\automation_sequencestart_child_new.ps1  -Action "Value2" 
Version History  
...
$vMsequencelist = ''
 ...
 #$action = 'Start'
         ...
         $subscriptions = Get-azSubscription -SubscriptionName $Subscription
            ...
            $vms = get-azvm -status | ? {$_.Tags.Keys -like "*Sequence*" }  # |select name, powerstate, tags, id
...
switch($Action) {
 ...
 foreach($sequence in $Sequncelist)
 {
